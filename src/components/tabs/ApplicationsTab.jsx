    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
        <div className={`${panelClass} bg-ink-900/60`}>
          ...
          <SkeletonBlock className="mt-2 h-10 w-full rounded-lg" />
          <SkeletonBlock className="mt-2 h-10 w-24 rounded-lg" />
        </div>
        <div className={`${panelClass} bg-ink-900/60`}>
          <SkeletonBlock className="h-4 w-32 rounded-full" />
          <SkeletonBlock className="mt-4 h-10 w-full rounded-lg" />
...
export default function ApplicationsTab({
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-3 sm:gap-4 md:flex-row md:items-center md:justify-between">
          <div className="space-y-1.5 sm:space-y-2">
            ...
            <p className="max-w-2xl text-sm text-slate-200/80">
              Uygulama kaydet, sec ve calistir.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <span className="inline-flex items-center rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-accent-200">
              Uygulama: {applications.length}
            </span>
            <span className="inline-flex items-center rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-accent-200">
              Backend map: {applicationBackendOptions.length}
            </span>
            <span className="inline-flex items-center rounded-full border border-white/10 bg-white/5 px-3 py-1 text-xs text-accent-200">
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1.1fr)_minmax(0,0.9fr)]">
        <section className={`${panelClass} bg-ink-900/60`}>
          ...
          <div className="mt-4 grid gap-3">
            <input
              ...
              className="h-10 rounded-lg border border-white/10 bg-ink-900 px-3 text-sm text-slate-100 placeholder:text-slate-500 focus:border-accent-400 focus:outline-none focus:ring-1 focus:ring-accent-500/30"
            />
            <textarea
              rows={3}
              value={appAboutDraft}
              ...
              className="resize-y rounded-lg border border-white/10 bg-ink-900 px-3 py-2 text-sm text-slate-100 placeholder:text-slate-500 focus:border-accent-400 focus:outline-none focus:ring-1 focus:ring-accent-500/30"
            />
            <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto]">
              <select
                ...
                className="h-10 appearance-none rounded-lg border border-white/10 bg-ink-900 px-3 text-sm text-slate-100 focus:border-accent-400 focus:outline-none focus:ring-1 focus:ring-accent-500/30 disabled:cursor-not-allowed disabled:opacity-60"
              >
                <option value="">
          ...
          <div className="mt-4 rounded-2xl border border-white/10 bg-ink-900/70 p-3">
            <p className="text-[11px] font-semibold uppercase tracking-[0.14em] text-slate-300">
              Kayitli Uygulamalar
            </p>
            {hasApplications ? (
              <div className="no-scrollbar mt-2 max-h-[220px] space-y-2 overflow-y-auto pr-1">
                  ...
                  return (
                    ...
                    >
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        ...
                        <span className="text-[11px] text-slate-400">{entry.backendLabel}</span>
                      </div>
              ...
              <p className="mt-2 text-[11px] text-slate-500">Henuz uygulama kaydi yok.</p>
            )}
          </div>
        </section>
        ...
        <section className={`${panelClass} bg-ink-900/60`}>
          ...
          <div className="mt-4 space-y-3">
            ...
            <div className="rounded-xl border border-white/10 bg-ink-900/70 p-3 text-[11px] text-slate-300">
              {selectedApplication ? (
                <>
                  <p className="font-semibold text-slate-100">{selectedApplication.name}</p>
                  <p className="mt-1 text-slate-400">{selectedApplication.about}</p>
                  <p className="mt-1 text-slate-400">Backend: {selectedApplication.backendLabel}</p>
                </>
              ) : (
                <p className="text-slate-500">Calistirmak icin bir uygulama secin.</p>
              )}
            </div>
      ...
      <section className={`${panelClass} overflow-hidden bg-ink-900/60`}>
