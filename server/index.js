...
const CRON_VIEW_PERMISSIONS = ["cron.view", "cron.manage", "cron.run", "cron.logs.view", "admin.manage"]
const CRON_MANAGE_PERMISSIONS = ["cron.manage", "admin.manage"]
const CRON_RUN_PERMISSIONS = ["cron.run", "cron.manage", "admin.manage"]
const CRON_LOGS_VIEW_PERMISSIONS = ["cron.logs.view", "cron.manage", "admin.manage"]
const CRON_LOGS_CLEAR_PERMISSIONS = ["cron.logs.clear", "cron.manage", "admin.manage"]
const CRON_DEFAULT_TIMEZONE = "Europe/Istanbul"
const CRON_DEFAULT_TIME = "00:05"
...
const getServerTimeZone = () =>
  String(process.env.TZ || Intl.DateTimeFormat().resolvedOptions().timeZone || CRON_DEFAULT_TIMEZONE)

const getTimeZoneParts = (date, timeZone = CRON_DEFAULT_TIMEZONE) => {
  try {
...
const shiftDateParts = (parts, dayOffset) => {
  const pivot = new Date(Date.UTC(parts.year, parts.month - 1, parts.day))
  pivot.setUTCDate(pivot.getUTCDate() + dayOffset)
  return {
...
const getTimeZoneOffsetMs = (date, timeZone) => {
  ...
  return interpretedUtc - date.getTime()
}

const zonedDateTimeToUtc = (parts, hour, minute, second, timeZone) => {
  ...
  if (refinedOffset !== offset) {
    timestamp = baseUtc - refinedOffset
  }
  return new Date(timestamp)
}

const normalizeCronJobType = (value) => {
  const normalized = String(value ?? "").trim().toLowerCase()
  return CRON_JOB_TYPES.has(normalized) ? normalized : ""
}

const normalizeCronScheduleTime = (value) => {
...
const normalizeCronTimezone = (value) => {
  const normalized = String(value ?? "").trim() || CRON_DEFAULT_TIMEZONE
  try {
    Intl.DateTimeFormat("en-US", { timeZone: normalized }).format(new Date())
    return normalized
  } catch {
    return CRON_DEFAULT_TIMEZONE
  }
}

const normalizeCronTargetDateMode = (value) => {
  const normalized = String(value ?? "").trim().toLowerCase()
...
const resolveCronTargetDate = (timeZone, targetDateMode) => {
  ...
  const dateParts =
    normalizeCronTargetDateMode(targetDateMode) === "today"
      ? nowParts
...
const computeCronNextRunAt = (jobLike, now = new Date()) => {
  const timeZone = normalizeCronTimezone(jobLike?.timezone)
  const scheduleTime = normalizeCronScheduleTime(jobLike?.scheduleTime)
  ...
  let candidate = zonedDateTimeToUtc(targetParts, hour, minute, 0, timeZone)
  if (candidate.getTime() <= now.getTime()) {
    targetParts = shiftDateParts(targetParts, 1)
    candidate = zonedDateTimeToUtc(targetParts, hour, minute, 0, timeZone)
  }
  return candidate
}

const isCronJobDue = (jobLike, now = new Date()) => {
  if (!jobLike?.isActive || jobLike?.isRunning) return false
  const scheduleTime = normalizeCronScheduleTime(jobLike?.scheduleTime)
  if (!scheduleTime) return false
  const timeZone = normalizeCronTimezone(jobLike?.timezone)
  const [hour, minute] = scheduleTime.split(":").map(Number)
  const nowParts = getTimeZoneParts(now, timeZone)
  const today = formatDateParts(nowParts)
  ...
  if (jobLike?.lastRunAt) {
    const lastRunParts = getTimeZoneParts(new Date(jobLike.lastRunAt), timeZone)
    if (formatDateParts(lastRunParts) === today) {
      return false
    }
  }
  return true
...
const serializeCronJob = (job) => {
  const nextRunAt = computeCronNextRunAt(job)
  return {
    id: job.id,
    name: job.name,
    type: job.type,
    ...
    nextRunAt: nextRunAt ? nextRunAt.toISOString() : job.nextRunAt ? job.nextRunAt.toISOString() : null,
...
const serializeCronRun = (run) => ({
  ...
  status: run.status,
  message: run.message || "",
  result: Number.isFinite(Number(run.result)) ? Number(run.result) : null,
...
const normalizeCronRunLogEntry = (status, message) => ({
  time: new Date().toISOString(),
  status: String(status ?? "").trim().toLowerCase() || "info",
...
const upsertSaleRecord = async (rawDate, rawAmount) => {
...
const updateCronJobNextRun = async (job) => {
...
const appendCronRunLog = async (runId, logs, status, message, extraData = {}) => {
...
const runEldoradoSayimSocket = async (targetDate, log) => {
  const automationConfig = await prisma.automationConfig.findUnique({
    where: { id: "default" },
    select: { wsUrl: true },
  })
  const wsBaseUrl = String(automationConfig?.wsUrl ?? "").trim()
  if (!wsBaseUrl) {
    throw new Error("Websocket adresi kayitli degil.")
  }
  const triggerUrl = buildSocketIoWsUrl(wsBaseUrl, {
    kind: "uygulama",
    backend: CRON_JOB_TYPE_ELDORADO_SAYIM,
    date: targetDate,
  })
  if (!triggerUrl) {
    throw new Error("Socket.IO adresi olusturulamadi.")
  }
  if (typeof WebSocket !== "function") {
    throw new Error("WebSocket istemcisi bu Node surumunde kullanilamiyor.")
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    const resetRunTimeout = (ms = 120000) => {
      ...
      timeoutId = setTimeout(() => {
        finish(new Error("Sayim sonucu beklenirken zaman asimi olustu."))
      }, ms)
    }
      ...
      for (const packet of packets) {
        ...
        if (packet.startsWith("0{")) {
          try {
            ...
            finish(new Error("Socket.IO baglantisi baslatilamadi."))
          }
          continue
        }
        ...
        if (packet.startsWith("40")) {
          hasConnected = true
          log("info", "eldoradosayim baglandi.")
        ...
        if (packet.startsWith("41")) {
          if (!hasResult) {
            finish(new Error("Baglanti sonuc gelmeden kapandi."))
          }
          return
        }

        if (packet.startsWith("44")) {
          finish(new Error("eldoradosayim tetiklenemedi."))
          return
        }
        ...
        if (eventName === "script-triggered" || eventName === "script-started") {
          log("info", "Sayim script baslatildi.")
          resetRunTimeout(300000)
          continue
        }
        ...
        if (eventName === "script-log") {
          const lines = String(firstArg?.message ?? "")
            ...
            .map((line) => line.trim())
            .filter(Boolean)
          const status = String(firstArg?.stream ?? "info").trim().toLowerCase() === "stderr" ? "error" : "info"
          lines.forEach((line) => log(status, line))
          resetRunTimeout(300000)
          continue
        }
        ...
        if (eventName === "sonuc") {
          ...
          if (!Number.isFinite(countValue) || countValue < 0) {
            finish(new Error("Sayim sonucu gecerli bir sayi olarak donmedi."))
            return
          }
          hasResult = true
          log("success", `Sayim alindi: ${countValue}`)
          finish(null, countValue)
          return
        }
        ...
        if (eventName === "script-exit") {
          ...
          if (Number.isFinite(exitCode) && exitCode !== 0 && !hasResult) {
            finish(new Error(`Script cikti. Kod: ${exitCode}`))
            return
          }
    ...
    socket.onerror = () => {
      finish(
        new Error(hasConnected ? "Baglanti sirasinda hata olustu." : "Websocket baglantisi kurulamadi."),
      )
    }
    ...
    socket.onclose = () => {
      ...
      if (hasConnected) {
        finish(new Error("Baglanti acildi ancak sonuc gelmedi."))
        return
      }
      finish(new Error("Websocket baglantisi kapandi."))
    }
  })
}
...
const executeCronJobRun = async (jobId, runId) => {
  ...
  try {
    ...
    const run = await prisma.cronJobRun.findUnique({ where: { id: runId } })
    if (!job || !run) {
      throw new Error("Cron gorevi bulunamadi.")
    }

    const pushLog = async (status, message) => {
      const entry = normalizeCronRunLogEntry(status, message)
      logs = [...logs, entry].slice(-CRON_RUN_LOG_LIMIT)
      await appendCronRunLog(runId, logs, "running", message)
    }
    ...
    const message = String(error?.message || "Cron gorevi tamamlanamadi.").trim()
    logs = [...logs, normalizeCronRunLogEntry("error", message)].slice(-CRON_RUN_LOG_LIMIT)
    const finishedAt = new Date()
    const refreshedJob = await prisma.cronJob.findUnique({ where: { id: jobId } }).catch(() => null)
    await prisma.$transaction([
      prisma.cronJobRun.update({
        where: { id: runId },
        data: {
    ...
    console.error("Cron job run failed", error)
  } finally {
    cronRuntimeState.runningJobs.delete(jobId)
  }
}

const queueCronJobRun = async (jobId, trigger = "manual") => {
  const job = await prisma.cronJob.findUnique({ where: { id: jobId } })
  if (!job) {
    throw new Error("Cron gorevi bulunamadi.")
  }
  ...
  const run = await prisma.$transaction(async (tx) => {
    const createdRun = await tx.cronJobRun.create({
      data: {
        jobId,
        trigger,
        targetDate,
        status: "queued",
        message: "Cron gorevi siraya alindi.",
        logs: initialLogs,
      },
    })
    await tx.cronJob.update({
      ...
      data: {
        ...
        lastRunMessage: "Cron gorevi calisiyor.",
        nextRunAt,
      },
    })
    return createdRun
  })
  executeCronJobRun(jobId, run.id).catch((error) => {
    console.error("Cron background run failed", error)
  })
  return run
}

const syncCronJobRuntimeState = async () => {
  ...
  const operations = jobs.map((job) =>
    prisma.cronJob.update({
      ...
      data: {
        isRunning: false,
        nextRunAt: job.isActive ? computeCronNextRunAt(job) : null,
        ...(job.isRunning
          ? {
              lastRunStatus: "error",
              lastRunMessage: "Sunucu yeniden baslatildi, calisan cron gorevi durduruldu.",
            }
          : {}),
      },
    }),
...
const pollCronJobs = async () => {
  if (cronRuntimeState.polling) return
  cronRuntimeState.polling = true
  try {
    const jobs = await prisma.cronJob.findMany({
      where: { isActive: true },
    ...
    for (const job of jobs) {
      if (!isCronJobDue(job)) continue
      try {
        await queueCronJobRun(job.id, "scheduled")
      } catch (error) {
        const message = String(error?.message || "").trim()
        if (message && message !== "Cron gorevi zaten calisiyor.") {
          console.error("Cron scheduler trigger failed", error)
        }
      }
    }
  } catch (error) {
    ...
    cronRuntimeState.polling = false
  }
}

const startCronScheduler = async () => {
  await syncCronJobRuntimeState()
  await pollCronJobs()
  if (cronRuntimeState.pollTimer) {
    clearInterval(cronRuntimeState.pollTimer)
  }
  cronRuntimeState.pollTimer = setInterval(() => {
    pollCronJobs().catch((error) => {
      console.error("Cron polling interval failed", error)
    })
...
app.get("/api/cron/jobs", requireAnyPermission(CRON_VIEW_PERMISSIONS), async (_req, res) => {
  const jobs = await prisma.cronJob.findMany({
...
app.post("/api/cron/jobs", requireAnyPermission(CRON_MANAGE_PERMISSIONS), async (req, res) => {
  const name = String(req.body?.name ?? "").trim()
  const type = normalizeCronJobType(req.body?.type)
  ...
  res.status(201).json(serializeCronJob(created))
})

app.put("/api/cron/jobs/:id", requireAnyPermission(CRON_MANAGE_PERMISSIONS), async (req, res) => {
  const id = String(req.params.id ?? "").trim()
  if (!id) {
    res.status(400).json({ error: "invalid id" })
    return
  ...
  const current = await prisma.cronJob.findUnique({ where: { id } })
  if (!current) {
    res.status(404).json({ error: "Cron job not found" })
    return
  }
  if (current.isRunning) {
    res.status(409).json({ error: "Cron job is running" })
    return
  }

  const name = req.body?.name === undefined ? current.name : String(req.body?.name ?? "").trim()
  const type = req.body?.type === undefined ? current.type : normalizeCronJobType(req.body?.type)
  const scheduleTime =
    req.body?.scheduleTime === undefined
      ? current.scheduleTime
      : normalizeCronScheduleTime(req.body?.scheduleTime)
  const timezone =
    req.body?.timezone === undefined ? current.timezone : normalizeCronTimezone(req.body?.timezone)
  const targetDateMode =
  ...
  res.json(serializeCronJob(updated))
})

app.delete("/api/cron/jobs/:id", requireAnyPermission(CRON_MANAGE_PERMISSIONS), async (req, res) => {
  const id = String(req.params.id ?? "").trim()
  if (!id) {
    res.status(400).json({ error: "invalid id" })
    return
...
app.post("/api/cron/jobs/:id/run", requireAnyPermission(CRON_RUN_PERMISSIONS), async (req, res) => {
  const id = String(req.params.id ?? "").trim()
  if (!id) {
    res.status(400).json({ error: "invalid id" })
    return
  ...
  try {
    const run = await queueCronJobRun(id, "manual")
    const job = await prisma.cronJob.findUnique({ where: { id } })
    res.status(202).json({
      job: job ? serializeCronJob(job) : null,
      run: serializeCronRun(run),
    })
  } catch (error) {
    const message = String(error?.message || "").trim()
    if (message === "Cron gorevi bulunamadi.") {
      res.status(404).json({ error: message })
      return
    }
    if (message === "Cron gorevi zaten calisiyor.") {
...
app.get("/api/cron/jobs/:id/runs", requireAnyPermission(CRON_LOGS_VIEW_PERMISSIONS), async (req, res) => {
  const id = String(req.params.id ?? "").trim()
  const limitRaw = Number(req.query?.limit)
  const take = Number.isFinite(limitRaw) ? Math.min(Math.max(Math.floor(limitRaw), 1), 100) : 20
  if (!id) {
    res.status(400).json({ error: "invalid id" })
    return
  }
  const [job, runs] = await Promise.all([
    prisma.cronJob.findUnique({ where: { id } }),
...
app.delete("/api/cron/jobs/:id/runs", requireAnyPermission(CRON_LOGS_CLEAR_PERMISSIONS), async (req, res) => {
  const id = String(req.params.id ?? "").trim()
  if (!id) {
    res.status(400).json({ error: "invalid id" })
    return
  ...
  await prisma.cronJobRun.deleteMany({ where: { jobId: id } })
  res.status(204).end()
})
